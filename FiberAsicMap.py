...
#####################################
# Form list of ASIC channels to map #
#####################################
...
def get_asic_ch(pt_idx, pt_pin):
    for i in asic_descr[pt_idx]:
    ...
    for i in dcb_descr[dcb_idx]:
        if '_ELK_CH' in i['Signal ID']:
            ...
            if i['Pigtail pin'] is not None:
                ...
                asic_ch = get_asic_ch(pt_idx, pt_pin)
                ...
                i['PT Attr'] = None
            single_dcb.append(i)
    gbtx_desc.append(single_dcb)

# Now do fiber mapping
